<template>
  <div class="page-wrapper">
    ...
    <div class="search-bar">
      ...
      <input v-model="search" type="text" placeholder="Cari akomodasi..." @input="onSearch" />
    </div>
    ...
    <div class="filters-container">
      ...
      <div class="filter-dropdown">
        <div class="filter-select" @click="showCategoryDropdown = !showCategoryDropdown; showFacilityDropdown = false; showLimitDropdown = false">
          <span class="filter-label">KATEGORI <span v-if="selectedCategories.length">({{ selectedCategories.length }})</span></span>
        </div>
      ...
      <div class="filter-dropdown">
        <div class="filter-select" @click="showFacilityDropdown = !showFacilityDropdown; showCategoryDropdown = false; showLimitDropdown = false">
          <span class="filter-label">FASILITAS <span v-if="selectedFacilities.length">({{ selectedFacilities.length }})</span></span>
        </div>
...
async function fetchData() {
  ...
  try {
    const url = search.value
      ? `${API_BASE}/api/v1/accommodations/?q=${encodeURIComponent(search.value)}`
      : `${API_BASE}/api/v1/accommodations/`
...
const facilityOptions = [
  { label: 'Air Panas/Dingin', value: 'hot_water' },
  { label: 'TV Kabel', value: 'tv_cable' },
  { label: 'Free Wifi', value: 'free_wifi' },
  { label: 'Restoran', value: 'restaurant' },
  { label: 'Kolam Renang', value: 'swimming_pool' },
  { label: 'Kebugaran', value: 'gym' },
  { label: 'Ruang Meeting', value: 'meeting_room' }
]
...
const filtered = computed(() => {
  ...
  if (selectedFacilities.value.length > 0) {
    // Memfilter akomodasi yang memiliki SEMUA fasilitas yang dipilih (AND logic)
